...
def one_user(user_id):
    if request.method == 'GET':
        ...
        db.session.commit()
        db.session.close()
        return f"Заказ с id {user_id} удален", 200
...
def one_order(order_id):
    if request.method == 'GET':
        ...
        order.start_date = order_data(year=year_start, month=month_start, day=day_start)
        order.end_date = order_data(year=int(year_end), month=int(month_end), day=int(day_end))
...
@app.route('/offers/<int:order_id>', methods=['GET', 'PUT', 'DELETE'])
def one_offer(offer_id):
    if request.method == 'GET':
        ...
        user = db.session.query(Offer).get(offer_id)
        if user is None:
            return "Пользователь не найден", 404
        db.session.delete(offers)
        db.session.commit()
        db.session.close()
        return f"Объект с id {offer_id} удален", 200
